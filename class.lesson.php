...
class Lesson
{
	function Lesson($title_string,$url=""){
		
		if(ord(substr($title_string, 0, 1))==38){//Remove "Ændret!"
			$x = preg_replace('/^.+\n/', '', $title_string);
	...
	##SUMMARY ADDITION
		...
		if(search($x,"Note:") AND search($x,"Lektier:")){
			$this->summary=$this->subject.$teacherinfo." - L:N";
		}
		if(!(search($x,"Note:") OR search($x,"Lektier:"))){
	...
	##BUILD DESC
		...
		if(search($x,"Note:") AND search($x,"Lektier:")){ // Both present
